    ...
    public class EmployeeController : BaseController
    {
        ...
        public async Task<IActionResult> Create(EmployeeViewModel vm)
        {
            ...
            if (string.IsNullOrWhiteSpace(vm.EmployeeNo))
            {
                ...
                do
                {
                    candidate = $"EMP-{count:D4}";
                    count++;
                }
                while (await _context.Employees.AnyAsync(e => e.EmployeeNo == candidate));
                vm.EmployeeNo = candidate;
            }
            else
            {
                bool duplicate = await _context.Employees
                    .AnyAsync(e => e.EmployeeNo == vm.EmployeeNo && e.EmployeeID != vm.EmployeeID);
                if (duplicate)
                {
